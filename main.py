...
def subject_details():
    query=db.engine.execute(f"SELECT subid,subname,fname FROM `subject`,`faculty` where subject.facid=faculty.id")

    return render_template('subject_details.html',query=query)
